...
class Discriminator(nn.Module):
    def __init__(self, nc=2, ndf=64, imsize=64, hflip=False):
    ...
    def forward(self, input, y=None):
        input = input[:, : self.nc]
        input = input.view(-1, self.imsize, self.imsize, self.nc).permute(
            0, 3, 1, 2
        )  # (BxN_samples)xC -> BxCxHxW

        if self.hflip:  # Randomly flip input horizontally
            input_flipped = input.flip(3)
            mask = (
                torch.randint(0, 2, (len(input), 1, 1, 1))
                .bool()
