...
from models.utils import combine_interleaved
...
class ResnetBlockFC(nn.Module):
    ...
    def __init__(self, size_in, size_out=None, size_h=None, beta=0.0):
        ...
        if beta > 0:
            self.activation = nn.Softplus(beta=beta, inplace=True)
        else:
...
class ResnetFC(nn.Module):
    ...
    def forward(self, zx, combine_inner_dims=(1,)):
        ...
        with profiler.record_function("resnetfc_infer"):
            ...
            if self.d_latent > 0:
                z = zx[..., : self.d_latent]  # [24576, 512
                x = zx[..., self.d_latent :]  # [24576, 78
            else:
                x = zx
            if self.d_in > 0:
                x = self.lin_in(x)  # [[24576, 512]
            else:
            ...
            for blkid in range(self.n_blocks):
                ...
                if self.d_latent > 0 and blkid < self.combine_layer:
                    tz = self.lin_z[blkid](z)  # [24576, 512]
                    if self.use_spade:
                ...
                x = self.blocks[blkid](x)  # [24576, 512]
