...
def gan_loss(pred, is_real):
    target = logic_tensor(pred, is_real)
    criterionBCE = torch.nn.BCELoss()
    loss = criterionBCE(pred, target)
    return loss


def logic_tensor(pred, is_real):
    if is_real:
        target = torch.tensor(1.0)
    else:
        target = torch.tensor(0.0)
    return target.expand_as(pred).to(pred.device)
